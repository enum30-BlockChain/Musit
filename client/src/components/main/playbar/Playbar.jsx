...
{
  /* <props MusicLikes address userList/> */
}
export const Playbar = () => {
  ...
  const MusicLikes = useSelector((state) => state.user).MusicLikes;
  ...
  useEffect(() => {
    ...
    console.log(MusicLikes);
    console.log(user.nickname);
    //첫로딩시 리센트 가져와서 세팅
    if (musicList.length > 0 && user.nickname) {
      ...
      if (!user.nickname) {
        ...
        if (user.recent_played === null) {
          ...
          const songs = MusicLikes;
          console.log(songs,"QmZKMrf2vcUGmnvvzAQrY2iSxPXQZRhBAitBAYRp78K1c8")
          const index = songs.findIndex((i) => i.ipfs_hash == arry[0]); //=한개쓰면 0,1만나오고 ==몇번째인지 나온다.
          console.log(index,"QmZKMrf2vcUGmnvvzAQrY2iSxPXQZRhBAitBAYRp78K1c8")
  ...
  function prevSong() {
    ...
    num--;
    if (num < 0) {
      num = MusicLikes.length - 1;
    }
    setCount(num);
    loadSong(MusicLikes[num]);
    playSong();
  }
  function nextSong() {
    let num = count;
    num++;
    if (num > MusicLikes.length - 1) {
      num = 0;
    }
    setCount(num);
    loadSong(MusicLikes[num]);
