...
export const createMusicData = (imgFormData, audioFormData, input) => {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      const imgUrl = (
        await axios.post("http://15.164.164.38/files/upload/img", imgFormData)
      ).data;

      const audioIpfsHash = (
        await axios.post(
          "http://15.164.164.38/files/upload/audio",
          audioFormData
        )
      ).data;
...
export const readMusicList = () => {
  return async (dispatch, getState) => {
    ...
    try {
      const url = "http://15.164.164.38/music/";
...
export const readMusicData = (ipfs_hash) => {
  return async (dispatch, getState) => {
    ...
    try {
      if (ipfs_hash) {
        const url = `http://15.164.164.38/music/${ipfs_hash}`;
        const musicList = (await axios.get(url)).data;
        dispatch({
...
export const readLikeMusicList = () => {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      const url = `http://15.164.164.38/music/likes/${accounts[0]}`;
      const musicList = (await axios.get(url)).data;
      dispatch({
...
export const updateMusicList = (input, hash) => {
  return async (dispatch, getState) => {
    dispatch({ type: ActionTypes.MUSIC_DATA_REQUEST });
    try {
      const url = `http://15.164.164.38/music/${hash}`;
    ...
    try {
      ...
      if (likeMusic && music.ipfs_hash) {
        ...
        if (0 >= likeFilter.length) {
          // 좋아요를 안눌렀으면 생성
          const url = `http://15.164.164.38/music/likes`;
          ...
          // 좋아요를 눌렀으면 다시 삭제
          const url = `http://15.164.164.38/music/likes/${music.ipfs_hash}`;
