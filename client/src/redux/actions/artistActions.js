  ...
  return async (dispatch, getState) => {
    ...
    try {
      ...
      if (accounts.length > 0) {
        const url = "http://15.164.164.38/artists/";
...
export const readArtistData = () => {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      let accounts = getState().metamask.accounts;
      if (accounts.length > 0) {
        const url = `http://15.164.164.38/artists/${accounts[0]}`;
  ...
  return async (dispatch, getState) => {
    ...
    try {
      ...
      if (likeArtist && artist_name) {
        if (0 >= update.length) {
          // 좋아요를 안눌렀으면 생성
          const url = `http://15.164.164.38/artists/likes`;
          ...
          // 좋아요를 눌렀으면 다시 삭제
          const url = `http://15.164.164.38/artists/likes/${artist_name}`;
...
export const readLikeArtistList = () => {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      if (accounts.length > 0) {
        const url = `http://15.164.164.38/artists/likes/${accounts[0]}`;
...
export const readLikeArtistDetail = (user_address) => {
  return async (dispatch, getState) => {
    ...
    try {
      if (user_address > 0) {
        const url = `http://15.164.164.38/artists/${user_address}`;
